...
pub fn store_capacity_buckets(graph: &CapacityGraph, directory: &Path, subfolder: String) -> Result<(), Box<dyn Error>> {
    ...
    let output_dir = directory.join("capacities").join(subfolder);
